...
To publish a release with all plugin binaries, run the GitHub Actions workflow [`Release plugins`](https://github.com/Great-Stone/vault-plugins/actions/workflows/release-plugins.yml) and provide a semver version (for example `v1.2.3`). The workflow cross-compiles all plugins for the supported targets, generates `SHA256SUMS`, tags the current `main` commit, and uploads assets to the GitHub Release.
